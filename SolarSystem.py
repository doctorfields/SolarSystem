...
w = window(width=1000,height=800,menus = False,title = 'Celestial Mechanics Simulator')
disp = display(window = w,x = 5,y = 5, width = 600, height = 750,forward = vector(0,3,1),range = 1.5*UA)
	...
	if SSCheckBox.GetValue():
		...
		for planet in SolarSystemObjects:
			if planet in CurrentPlanets:
				planet.trail_object.visible = False
...
def Preparados(evt):
	...
	NewPlanet = Planet(pos = posicion_inicial,mass = mass,real_radius = real_radius,virtual_radius = virtual_radius,color = (1,0,1))
...
def YA(evt):
	...
	if not TrailsCheckBox.GetValue():
		NewPlanet.trail_object.visible = False
...
def ShowPlanet(evt,planet):
	global SolarSystemObjects,SSOCheckBoxes
	if SSOCheckBoxes[planet.name].GetValue():
		planet.visible = True
		if planet.name != 'Asteroid belt':
			CurrentPlanets.append(planet)
			if TrailsCheckBox.GetValue(): 
...
# Sun
Sun = Planet(name ="Sun",real_radius =6.96e8,virtual_radius = 10e9,mass = 1.989e30,color = (1,1,0))
# Planets
Mercury = Planet(name = "Mercury",real_radius = 2.4397e6,virtual_radius = 2e9,color = (0.7,0.7,0.7))
Mercury.SetPlanet(mass = 3.302e23,major_semiaxis = 0.387098*UA,excentricity = 0.20563069)		
Venus = Planet(name = "Venus",real_radius = 6.0518e6,virtual_radius = 5e9,color = (1,0.8,0))
Venus.SetPlanet(mass = 4.869e24,major_semiaxis = 0.723327*UA,excentricity = 0.00677323)
Earth = Planet(name = "Earth",real_radius = 6.3710e6,virtual_radius = 6e9,material = materials.BlueMarble)
Earth.SetPlanet(mass = 5.98e24,major_semiaxis = 149598261e3,excentricity = 0.01671123)
Mars = Planet(name = "Mars",real_radius = 3.3895e6,virtual_radius = 4e9,color = (0.8,0,0))
Mars.SetPlanet(mass = 6.4185e23,major_semiaxis = 1.523679*UA,excentricity = 0.093315)
Jupiter = Planet(name = "Jupiter",real_radius = 69.911e6,virtual_radius = 15e10,color = (0.8,0.6,0))
Jupiter.SetPlanet(mass = 1.899e27,major_semiaxis = 5.204267*UA,excentricity = 0.04839266)		
Saturn = Planet(name = "Saturn",real_radius = 58.232e6,virtual_radius = 12e10,color = (0.6,0.5,0))
Saturn.SetPlanet(mass = 5.688e26,major_semiaxis = 9.5820172*UA,excentricity = 0.05415060)
Uranus = Planet(name = "Uranus",real_radius = 25.362e6,virtual_radius = 10e10,color = (0.6,0.6,1))
Uranus.SetPlanet(mass = 8.686e25,major_semiaxis = 19.22941195*UA,excentricity = 0.044405586)		
Neptune = Planet(name = "Neptune",real_radius = 24.622e6,virtual_radius = 10e10,color = (0,0,0.8))
Neptune.SetPlanet(mass = 1.024e26,major_semiaxis = 30.10366151*UA,excentricity = 0.00858587)
# Asteroid_Belt
Asteroid_Belt = extrusion(pos = Sun.pos,shape=shapes.circle(radius = 2.855*UA,np=64,thickness = 0.795*UA),color=(0.1,0.15,0));
...
p = w.panel
wx.StaticText(p,label = "Time Speed",pos = (650,20))
SpeedSlider = wx.Slider(p,pos = (650,50),size = (200,20),value = dt,minValue = -50000,maxValue = 200000)
SpeedSlider.Bind(wx.EVT_SCROLL,SetRythm)
...
for i in range(len(SolarSystemObjects)):
	...
	exec "SSOCheckBoxes[planet.name].Bind(wx.EVT_CHECKBOX,lambda evt: ShowPlanet(evt,SolarSystemObjects["+str(i)+"]))"
